import { handle } from "../../../src/handlers/todo.completed.js";

jest.spyOn(process, "exit").mockImplementation(() => {
  return undefined as never;
});

jest.mock("axios", () => {
  return {
    post: jest.fn(
      () =>
        new Promise((resolve, reject) => {
          resolve({
            data: {
              update_todos_by_pk: {
                id: "test-1",
                completed: true
              },
            },
          });
        })
    ),
  };
});

describe("todo.completed event handler", () => {
  it("should export minimum exports", () => {
    expect(handle).toBeDefined();
    expect(typeof handle === "function").toBe(true);
  });

  it("should handle event", async () => {
    const request = {
      body: {},
      log: {
        info: jest.fn(),
      },
    };
    const reply = {
      status: jest.fn(() => ({
        send: jest.fn(),
        json: jest.fn(),
      })),
    };
    const event = {
      type: "todo.completed",
      data: {
        id: "test-1",
      },
      source: "tests",
    };
    await handle(request, reply, event);
  });

  it("should handle event sync", async () => {
    const request = {
      body: {},
      log: {
        info: jest.fn(),
      },
    };
    const reply = {
      status: jest.fn(() => ({
        send: jest.fn(),
        json: jest.fn(),
      })),
    };
    const event = {
      type: "todo.completed",
      data: {
        id: "test-1",
      },
      source: "tests",
    };
    await handle(request, reply, event, { sync: true });
  });

  it("should handle event that has already been processed", async () => {
    const request = {
      body: {},
      log: {
        info: jest.fn(),
      },
    };
    const reply = {
      status: jest.fn(() => ({
        send: jest.fn(),
        json: jest.fn(),
      })),
    };
    const event = {
      type: "todo.completed",
      data: {
        id: "test-1",
        completedDenormalizers: ["example-hasura"],
      },
      source: "tests",
    };
    await handle(request, reply, event);
  });

  it("should handle event that has already been processed - sync", async () => {
    const request = {
      body: {},
      log: {
        info: jest.fn(),
      },
    };
    const reply = {
      status: jest.fn(() => ({
        send: jest.fn(),
        json: jest.fn(),
      })),
    };
    const event = {
      type: "todo.completed",
      data: {
        id: "test-1",
        completedDenormalizers: ["example-hasura"],
      },
      source: "tests",
    };
    await handle(request, reply, event, { sync: true });
  });

  it("should handle hasura error", async () => {
    const request = {
      body: {},
      log: {
        info: jest.fn(),
      },
    };
    const reply = {
      status: jest.fn(() => ({
        send: jest.fn(),
        json: jest.fn(),
      })),
    };
    const event = {
      type: "todo.completed",
      data: {
        id: "test-1",
      },
      source: "tests",
    };
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const axios = require("axios");
    axios.post.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          resolve({
            data: {
              errors: ["Fake error"],
            },
          });
        })
    );
    await handle(request, reply, event);
  });
});
